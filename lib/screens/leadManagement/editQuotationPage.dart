...
class EditQuotationPage extends StatefulWidget {
  ...
  const EditQuotationPage({Key? key, required this.item}) : super(key: key);
...
class _EditQuotationPageState extends State<EditQuotationPage> {
 ...
 details.QuotationTemplateDetailsModel? _templateDetailsModel;
  ...
  Future<void> _loadQuotationData() async {
    setState(() => isLoadingQuotation = true);
    final httpService = HttpService();
    final quotation = await httpService.getQuotationEdit(widget.item);

    if (quotation != null && quotation.data != null) {
      setState(() {
      ...
      setState(() => isLoadingQuotation = false);
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text("Failed to load quotation data")),
      );
    }
  }

  void _populateFormData() {
    if (_quotationData?.data?.quotationData?.isNotEmpty ?? false) {
      final quote = _quotationData!.data!.quotationData!.first;
      quotationId = quote.quoteId ?? "";
      selectedCustomerId = quote.customerId;
      selectedWorkOrder = quote.workorderId;
      selectedTemplateId = quote.templateId;
      ...
      final customer = _customerModel?.data.firstWhere(
        ...
        orElse: () => CustomerDetails(
            id: '', name: '', emailId: '', contactNo: '', address: ''),
      );
      if (customer != null && customer.id.isNotEmpty) {
        _customerController.text = customer.name;
      }

      if (quote.enquiryDate != null) {
        final date = DateTime.tryParse(quote.enquiryDate!);
        if (date != null) {
          enquiryDateController.text = DateFormat('dd-MM-yyyy').format(date);
        }
      }

      addressController.text = quote.address ?? "";
      districtController.text = quote.district ?? "";
      stateController.text = quote.state ?? "";
      nationalityController.text = quote.nationality ?? "";

      // Match state and district from the quotation data
      _matchStateAndDistrict(quote.state ?? "", quote.district ?? "");

      if (selectedTemplateId != null && selectedTemplateId!.isNotEmpty) {
        _loadTemplateDetails(selectedTemplateId!);
      }

      if (selectedCustomerId != null && selectedCustomerId!.isNotEmpty) {
        _loadWorkOrderId(selectedCustomerId!);
      }

      _populateProductRows();
    }
  }

  void _matchStateAndDistrict(String stateName, String districtName) {
    if (stateName.isNotEmpty && stateList.isNotEmpty) {
      // Find matching state
      ...
      if (matchedState.id.isNotEmpty) {
        ...
        // Load districts for the matched state
  ...
  void _populateProductRows() {
    if (_quotationData?.data?.itemData != null) {
      productRows.clear();
      for (var item in _quotationData!.data!.itemData!) {
        MaterialData? materialData;
        if (materials.isNotEmpty && item.item != null) {
          try {
            materialData = materials.firstWhere(
              (material) => material.materialId == item.item,
            );
          } catch (e) {
            materialData = null;
          }
        ...
        // Initialize material controller for this product row
        final materialCtrl = TextEditingController(
          text: materialData?.materialName ?? '',
        );
        _materialControllers[productRows.length] = materialCtrl;
        ...
        productRows.add(
          ProductRow(
            ...
            gstController: TextEditingController(text: item.gst ?? '0'),
  ...
  Future<void> _loadCustomers({bool showLoading = true}) async {
    ...
    if (customerList != null && customerList.data.isNotEmpty) {
      setState(() {
        ...
        // Set customer name in controller if data is loaded
        if (_quotationData != null) {
          _populateFormData();
        }
      });
  ...
  void _showCustomerSearchDialog(List<CustomerDetails> customers) {
    ...
    showDialog(
      ...
      builder: (BuildContext context) {
        ...
        List<CustomerDetails> filteredList = List.from(_customerModel!.data);
                ...
                child: Column(
                  children: [
                    TextField(
                      ...
                      onChanged: (value) {
                        setDialogState(() {
                          filteredList = _customerModel!.data
                              .where((c) => c.name
                                  .toLowerCase()
  ...
  Future<void> _refreshAfterCustomerAdded() async {
    ...
    try {
      ...
      // Show the updated search dialog
      if (mounted) {
        await Future.delayed(const Duration(milliseconds: 300));
        _showCustomerSearchDialog(_customerModel?.data ?? []);
      }
    ...
    try {
      ...
      final productList = productRows.map((row) {
        return {
          ...
          "unit": row.materialData?.unitName ?? row.originalUnit ?? '',
        };
      }).toList();
      ...
      final response = await httpService.updateQuotation(formData);
                ...
                child: Column(
                  children: [
                    TextField(
                      ...
                      onChanged: (value) {
                        setDialogState(() {
                          filteredList = _templateModel!.data
  ...
  Future<void> _loadTemplateDetails(String templateId) async {
    ...
    if (templateDetails != null &&
      ...
      setState(() {
        _templateDetailsModel = templateDetails;
        isLoadingTemplates = false;
      });
    } else {
      setState(() => isLoadingTemplates = false);
  ...
  void _addProductRow() {
    setState(() {
      ...
      // Initialize controller for new row
      _materialControllers[productRows.length - 1] = TextEditingController();
    });
  }
  ...
  void _removeProductRow(int index) {
    setState(() {
      ...
      _materialControllers.remove(index);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        centerTitle: true,
        backgroundColor: const Color.fromARGB(255, 40, 138, 204),
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        if (_templateDetailsModel != null &&
                          ...
                          Column(
                            crossAxisAlignment: CrossAxisAlignment.start,
                            children: _templateDetailsModel!.data!.fields!.map((
                              field,
                            ) {
                              final existingField = _quotationData
                                  ?.data?.quotationPackageEst
                                  ?.firstWhere(
                                (est) => est.fieldId == field.fieldId,
                                orElse: () => QuotationPackageEst(),
                              );

                              final controller = TextEditingController(
                                text: existingField?.fieldValue ??
                                    field.fieldData ??
                                    "",
                              );

                              // Check if field should be non-editable (try both string and int)
                              ...
                              if (field.fieldId != null) {
                                if (field.fieldId is String) {
                                  ...
                                  isNonEditable =
                                      field.fieldId == 1 || field.fieldId == 2;
                                }
                              }
                        ...
                        Align(
                          ...
                          child: ElevatedButton.icon(
                            ...
                            label: const Text("Update Quotation"),
                            style: ElevatedButton.styleFrom(
  ...
  Widget _buildProductRow(int index, ProductRow row) {
    ...
    if (!_materialControllers.containsKey(index)) {
      _materialControllers[index] = TextEditingController(
        text: row.materialData?.materialName ?? '',
      );
    }
    ...
    return Padding(
      ...
      child: Row(
        key: Key('product_row_${index}_${row.materialId ?? 'empty'}'),
        children: [
          Expanded(
            flex: 1,
          ...
          Expanded(
            ...
            child: TextFormField(
              ...
              onTap: () {
                _showMaterialSearchDialog(index);
              },
            ),
          ),
          SizedBox(width: 4),
  ...
  Widget _textField({
    ...
    if (hint != null && textController.text.isEmpty) {
      textController.text = hint;
    }
  ...
  void _showMaterialSearchDialog(int productIndex) {
    TextEditingController searchCtrl = TextEditingController();
                          ...
                          : ListView.builder(
                              ...
                              itemBuilder: (context, index) {
                                ...
                                return InkWell(
                                  onTap: () {
                                    setState(() {
                                      productRows[productIndex].materialData =
                                          material;
                                      productRows[productIndex].materialId =
                                          material.materialId;
                                      _materialControllers[productIndex]!.text =
                                          material.materialName ?? '';
                                      productRows[productIndex]
                                          .rateController
                                          .text = material.unitPrice ?? '0.00';
                                    });
                                    Navigator.pop(context);
                                  },
                                  child: Container(
