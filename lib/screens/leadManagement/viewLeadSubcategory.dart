...
class _ViewLeadSubCategoryState extends State<ViewLeadSubCategory>
  ...
  ViewLeadSubCategoryModel? _viewSubLeadsCategory;
  ...
  Future<void> _getData() async {
    ...
    if (hasInternet) {
      _viewSubLeadsCategory = await HttpService.viewLeadsSubCategory(
        widget.token,
        widget.categoryId,
      );
      if (_viewSubLeadsCategory != null && mounted) {
        setState(() {});
      }
    }
  }
  ...
  void _showAddSubCategoryDialog() {
    ...
    showGeneralDialog(
      barrierLabel: "addSubCategoryDialog",
  ...
  Future<void> _addSubCategory() async {
    if (_subcategoryController.text.trim().isEmpty) {
      Common.toastMessaage('Please enter a subcategory name', Colors.red);
      return;
    }

    Common.showProgressDialog(context, "Creating...");
    final addCategory = await HttpService.addLeadSubCategory(
      widget.token,
  ...
  void _showEditSubCategoryDialog(String id, String name) {
    _editSubcategoryController.text = name;
    showGeneralDialog(
      barrierLabel: "editSubCategoryDialog",
      barrierDismissible: true,
      barrierColor: Colors.black.withOpacity(0.5),
      transitionDuration: const Duration(milliseconds: 300),
      context: context,
                    ...
                    child: Column(
                      children: [
                        ...
                        SizedBox(
                          ...
                          height: 52,
                          child: ElevatedButton(
                            onPressed: () => _updateSubCategory(id),
                            style: ElevatedButton.styleFrom(
                              backgroundColor:
                                  const Color.fromARGB(255, 47, 131, 180),
                              foregroundColor: Colors.white,
  ...
  Future<void> _updateSubCategory(String id) async {
    if (_editSubcategoryController.text.trim().isEmpty) {
      Common.toastMessaage('Please enter a subcategory name', Colors.red);
      return;
    }

    Common.showProgressDialog(context, "Updating...");
    final editSubCategory = await HttpService.editLeadSubCategory(
      widget.token,
      _editSubcategoryController.text.trim(),
      id,
    );
    ...
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        actions: [
          ...
          TextButton(
            onPressed: () => _deleteSubCategory(id, ctx),
            child: const Text(
              'Delete',
  ...
  Future<void> _deleteSubCategory(String id, BuildContext ctx) async {
    Navigator.pop(ctx);
    Common.showProgressDialog(context, "Deleting...");
    final deleteCategory =
        await HttpService.deleteLeadSubCategory(widget.token, id);
  ...
  Widget build(BuildContext context) {
    return RefreshIndicator(
      ...
      color: const Color.fromARGB(255, 47, 131, 180),
      child: Scaffold(
  ...
  Widget _buildBody() {
    ...
    final subcategories = _viewSubLeadsCategory!.data;
    ...
    return ListView.separated(
      ...
      itemBuilder: (context, index) {
        ...
        return _buildSubCategoryCard(
          id: subcategory.leadSubCategoryId.toString(),
          name: subcategory.leadSubCategory.toString(),
          index: index,
        );
      },
    );
                ...
                children: [
                  if (widget.updateLeads) ...[
                    _buildActionButton(
                      ...
                      onTap: () => _showEditSubCategoryDialog(id, name),
                    ),
        ...
        children: [
          ...
          if (widget.createLeads)
            ElevatedButton.icon(
              ...
              label: const Text('Create Subcategory'),
              style: ElevatedButton.styleFrom(
