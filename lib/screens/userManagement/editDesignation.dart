...
class _EditDesignationPageState extends State<EditDesignationPage> {
  ...
  getData() async {
    ...
    menuModel =
        await HttpService.editMenuList(widget.token, widget.designationId);
    if (menuModel != null) {
      setState(() {
        ...
        for (int i = 0; i < menuModel!.data!.menuList!.length; i++) {
          if (menuModel!.data!.menuList![i].isAvailable == true) {
            for (int j = 0;
                j < menuModel!.data!.menuList![i].subMenu!.length;
                j++) {
              if (menuModel!.data!.menuList![i].subMenu![j].isChecked == true) {
                setState(() {
                  checkedItems
                      .add(menuModel!.data!.menuList![i].subMenu![j].id);
                  sts = true;
                });
              } else {
                setState(() {
                                  ...
                                  ExpansionPanel(
                                      body: MediaQuery.removePadding(
                                        ...
                                        child: ListView.builder(
                                          ...
                                          itemCount: menuModel!.data!
                                              .menuList![index].subMenu!.length,
                                        ),
                                      ),
                          ...
                          onTap: () async {
                            if (designation.text.isEmpty) {
                              Common.toastMessaage(
                                  'Designation Cannot empty', Colors.red);
                            } else {
                              ...
                              PostEditSubmenuModel postSubmenu =
                                  await HttpService.postEditSubMenu(body);
                              if (postSubmenu.data == true) {
                                Common.toastMessaage(
                                    postSubmenu.message, Colors.green);
                                if (context.mounted) {
