...
StaffCalldetailsModel staffCalldetailsModelFromJson(String str) => StaffCalldetailsModel.fromJson(json.decode(str));

String staffCalldetailsModelToJson(StaffCalldetailsModel data) => json.encode(data.toJson());
...
class StaffCalldetailsModel {
    ...
    factory StaffCalldetailsModel.fromJson(Map<String, dynamic> json) => StaffCalldetailsModel(
...
class Data {
    ...
    factory Data.fromJson(Map<String, dynamic> json) => Data(
        ...
        callCountByResponse: List<CallCountByResponse>.from(json["callCountByResponse"].map((x) => CallCountByResponse.fromJson(x))),
        leadStatusGraph: List<LeadStatusGraph>.from(json["lead_status_graph"].map((x) => LeadStatusGraph.fromJson(x))),
        leadCategory: List<String>.from(json["lead_category"].map((x) => x)),
        leadCategoryCount: List<LeadCategoryCount>.from(json["lead_category_count"].map((x) => LeadCategoryCount.fromJson(x))),
    );

    Map<String, dynamic> toJson() => {
        "callDetails": callDetails.toJson(),
        "callCountByResponse": List<dynamic>.from(callCountByResponse.map((x) => x.toJson())),
        "lead_status_graph": List<dynamic>.from(leadStatusGraph.map((x) => x.toJson())),
        "lead_category": List<dynamic>.from(leadCategory.map((x) => x)),
        "lead_category_count": List<dynamic>.from(leadCategoryCount.map((x) => x.toJson())),
    };
}
...
class CallCountByResponse {
    ...
    factory CallCountByResponse.fromJson(Map<String, dynamic> json) => CallCountByResponse(
...
class LeadCategoryCount {
    ...
    LeadCategoryCount({
        required this.leadCategory,
        required this.the1Count,
        required this.the2Count,
        required this.the3Count,
        required this.the4Count,
        required this.the5Count,
    });

    factory LeadCategoryCount.fromJson(Map<String, dynamic> json) => LeadCategoryCount(
...
class LeadStatusGraph {
    ...
    factory LeadStatusGraph.fromJson(Map<String, dynamic> json) => LeadStatusGraph(
        resCount: json["res_count"],
