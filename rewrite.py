...
class _AddLeadsNewState extends State<AddLeadsNew> {
  ...
  LeadProductSectionModel? leadProductsModel;
  List<LeadProduct> productsList = [];
  List<LeadProduct> selectedProducts = [];
  String productsText = \'Select Products\';
  TextEditingController productsVal = TextEditingController();
  ...
  getData() async {
    ...
    leadProductsModel = await HttpService.leadProductSection();
    if (leadProductsModel != null && leadProductsModel!.status == true) {
      productsList = leadProductsModel!.data ?? [];
    }
  }
